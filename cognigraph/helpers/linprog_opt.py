...
import logging
logger = logging.getLogger('LINPROG')
...
def _do_step(x, y, z, tau, kappa, d_x, d_y, d_z, d_tau, d_kappa, alpha):
    ...
    y = y + alpha * d_y
    logger.debug('Finish do step in {:.2f}'.format((time() - prev) * 1000))
...
def _display_iter(rho_p, rho_d, rho_g, alpha, rho_mu, obj, header=False):
    ...
    """
    if header:
        logger.debug("Primal Feasibility ",
                     "Dual Feasibility   ",
                     "Duality Gap        ",
                     "Step            ",
                     "Path Parameter     ",
    ...
    logger.debug(fmt.format(rho_p, rho_d, rho_g, alpha, rho_mu, obj))
    ...
    while go:
        ...
        try:
            ...
            if ip:  # initial point
                ...
                logger.debug('Finish _get_step'
                             ' in {:.2f}'.format((time() - prev) * 1000))
                # [1] Equation 8.9
                x, y, z, tau, kappa = _do_step(
        ...
        rho_p, rho_d, rho_A, rho_g, rho_mu, obj = _indicators(
            A, b, c, c0, x, y, z, tau, kappa)
        logger.debug('Finished _indicators in {:.2f}'.format((time() - prev) * 1000))
        prev = time()
        ...
        if inf1 or inf2:
            ...
            break
        logger.debug(
            'Finished all the rest in {:.2f}'.format((time() - prev) * 1000))
        # prev = time()

    logger.info(
        'Optimization finished in {:.2f}'.format((time() - prev) * 1000))
    if disp:
        logger.info(message)
...
def _linprog_ip(
    ...
    logger.debug('Prepare for presolving in {:.2f}'.format((time() - t_ini) * 1000))
    prev = time()
    ...
    logger.debug('Presolved in {:.3f}'.format((time() - prev) * 1000))
    prev = time()
    ...
    logger.debug('Solve in {:.3f}'.format((time() - prev) * 1000))
    prev = time()
    ...
    logger.debug('Postprocess in {:.3f}'.format((time() - prev) * 1000))
    prev = time()
    sol = {
        'x': x,
        'fun': fun,
