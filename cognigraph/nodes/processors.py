...
class Preprocessing(ProcessorNode):
    ...
    def _update(self):
        ...
        if not enough_collected:
            if self.parent_node.output is not None and\
                    self.parent_node.output.shape[TIME_AXIS] > 0:
        ...
        if self._dsamp_freq and self._dsamp_freq < self.mne_info['sfreq']:
            raw = mne.io.RawArray(self.parent_node.output, self.mne_info)
    ...
    def _update_statistics(self):
        input_array = self.parent_node.output.astype(np.dtype('float64'))
...
class InverseModel(ProcessorNode):
    ...
    def _update(self):
        ...
        input_array = self.parent_node.output
...
class LinearFilter(ProcessorNode):
    ...
    def _update(self):
        input_data = self.parent_node.output
    ...
    def _update(self):
        ...
        if not enough_collected:
            ...
            self.output[self._good_ch_inds, :] = np.dot(
                    self._ica_rejector,
                    self.parent_node.output[self._good_ch_inds, :])
