...
from cognigraph import ANATOMY_DIR
from cognigraph import FORWARDS_DIR
import ssl
...
class _SafeConnection:
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        if exc_type in (HTTPError, URLError):
            self._logger.error(
                "Connection problem: %s, %s", exc_value, exc_traceback
            )
...
class DataDownloader:
    ...
    def _fetch_fname(self, fname, group="files"):
        with open(self._config_path, "r") as f:
            download_config = json.load(f)
            dl_file = download_config[group][fname]
    ...
    def _download(self, url, dest_path, md5_hash, blocksize=DEFAULT_BLOCKSIZE):
        ...
        with _SafeConnection(self._logger) as s:
            with urlopen(
                url, timeout=5, context=_context
            ) as r, NamedTemporaryFile(delete=False) as f:
                ...
                while True:
                    ...
                    if not chunk:
                        break
                    self._dl_progress(
                        op.basename(dest_path), i_block, blocksize, totalsize
                    )
    ...
    def _get_download_url(self, url_pub):
        ...
        with _SafeConnection(self._logger):
            with urlopen(
                self._API_ENDPOINT % url_pub, timeout=10, context=_context
            ) as r:
